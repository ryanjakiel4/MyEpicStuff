...
public class NewGameEndless2 extends Canvas
implements KeyListener, Runnable
{
   ...
   private boolean hasQuiver = true, quiverAppeared = false, shootingArrow = false, arrowShot = false;
   ...
   public NewGameEndless2(Dimension size)
   {
         ...
         if(introduction)
         {
            ...
            else
            {
               ...
               if(startGame)
               {
                  ...
                  heroY += platformSpeed;
                  if(backgroundImagePlace < -1*(level*levelDuration))
                  {
                     level++;
                  ...
                  if(generalCount % 35 == 0 && backgroundImagePlace < -5)
                  {
                     if(!gameover)
                     {
            ...
            if(indexOfCollision == -1)
            {
               ...
               if(arrowShot)
               {
                  ...
                  else
                  {
                     for(int n = 0; n < likeArray.size(); n++)
                     {
                        if(arrowX - Math.abs((likeArray.get(n).getX() + 20)) <= arrowSpeed && Math.abs(likeArray.get(n).getY() - arrowY) < 15)
                        {
                           likeArray.remove(n);
                           if(Math.random()*level > .9)
                           {
   ...
   public void Draw()
   {
      ...
      else
      {
         try
         {
            ...
            // 
            // }
            if(backgroundImagePlace > -1*beginningBackground.getHeight())
            {
