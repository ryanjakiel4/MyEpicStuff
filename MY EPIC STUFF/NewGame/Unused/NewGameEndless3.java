...
public class NewGameEndless3 extends Canvas
implements KeyListener, Runnable
{
   private int speedOfPlay = 10;
   ...
   public NewGameEndless3(Dimension size)
   {
      ...
      if(!displayingText)
      {
         if(introduction)
         {
            ...
            else
            {
               ...
               if(startGame)
               {
                  backgroundImagePlace -= (heroY-oldHeroY);
            ...
            */
            ...
            if(gameover)
            {
               if(backgroundImagePlace == 0)
               {
                  ...
                  if(newHighScore)
                  {
                     bufferGraphics.drawString("New Highscore!", 220,470);
                  }
                  bufferGraphics.drawString("Highscore: "+high, 220 ,505);
                  if(linkDeadCount < 72)
                     linkDeadCount++;
               }
            }
   ...
   public void instantiateNewPlatforms()
   {
      ...
      while(surfaces.size() < WINDOWHEIGHT/platformSeparation)
      {
         ...
         else
         {
            ...
            if(level >= 4 && hasQuiver && !arrowAppeared && Math.random() > .99 && numberOfArrows < 10)
            {
   ...
   public void displayText(int w, Graphics bufferGraphics)
   {
      if(!secondGame)
      {
         if(message[w].length > displayTextCount + 1)
         {
            ...
            bufferGraphics.drawString("press z to continue",350,445);
         }
         else
         {
            displayingText = false;
