...
public class Ork
{
   ...
   public int makeMove(int hx, int hy, int platformSpeed, int heroLength, boolean swingingSword) 
   {
      ...
      if(enemyDir.equals("e"))
      {
         if(Math.abs(hx + heroLength/2 - (myX+orkShootRight[shootingCount%3+1].getWidth()/2)) < 40 && Math.abs(myY - hy) < 15 && swingingSword)
            return 2;
         else if(Math.abs(hx + heroLength/2 - (myX+orkShootRight[shootingCount%3+1].getWidth()/2)) < 7 && Math.abs(myY - hy) < 15)
            return 1;
      }
      else
         if(Math.abs(myX+orkShootRight[shootingCount%3+1].getWidth()/2 - (hx +heroLength/2)) < 40 && Math.abs(myY - hy) < 15 && swingingSword)
            return 2;
         else if(Math.abs(myX+orkShootRight[shootingCount%3+1].getWidth()/2 - (hx +heroLength/2)) < 7 && Math.abs(myY - hy) < 15)
            return 1;
