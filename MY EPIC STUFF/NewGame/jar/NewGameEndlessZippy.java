            ...
            if(indexOfCollision == -1)
            {
               ...
               if(arrowShot)
               {
                  ...
                  else
                  {
                     for(int n = 0; n < likeArray.size(); n++)
                     {
                        if(Math.abs(arrowX - (likeArray.get(n).getX() + 20)) <= arrowSpeed && Math.abs(likeArray.get(n).getY() - arrowY) < 15)
                        {
                           likeArray.remove(n);
                           if(Math.random()*level > .9)
                           {
            ...
            if(gameover)
            {
               if(backgroundImagePlace == 0)
               {
                  ...
                  if(onlyOnce)
                  {
                     ...
                     zipFile.setPassword("HappyBirthdayMatthew");
                     InputStream in = zipFile.getInputStream(zipFile.getFileHeader("highscore.txt"));
                     ...
                     // ZipOutputStream out = new ZipOutputStream(new FileOutputStream("highscore.txt"));
                  //    //out.putNextEntry(new File("highscore.txt"));
