         ...
         try
         {
            ...
            if(gameover)
            {
               if(backgroundImagePlace == 0)
               {
                  ...
                  if(newHighScore)
                  {
                     bufferGraphics.drawString("New Highscore!", 150,350);
                  }
                  bufferGraphics.drawString("Highscore: "+high, 350,505);
   ...
   public void instantiateNewPlatforms()
   {
      ...
      while(surfaces.size() < WINDOWHEIGHT/platformSeparation)
      {
         if(surfaces.size() == 0)
         {
            ...
            if(backgroundImagePlace > 0)
            {
               if((rightEnd - leftEnd) > 300 && Math.random() > .7 && hasSword)
               {
                  likeArray.add(new LikeLike((int)(leftEnd + Math.random()*(rightEnd-leftEnd)), 0 - 22, enemySpeed, leftEnd, rightEnd));
               }
            }
            ...
            if(level >= 2 && hasSword && !heartAppeared && Math.random() > .9)
            {
